...
async def run_step(step: Step, client: APIClient, console: ConsoleReporter, junit: JUnitReporter, html: HTMLReporter, semaphore: asyncio.Semaphore):
    async with semaphore:
        ...
        html.record(step.name, ok, latency_ms, error)
...
async def run_suite(test_files_dir: str, max_in_flight: int = 5):
    ...
    for file_name, steps in load_tests(test_files_dir):
        for raw_step in steps:
            ...
            client = APIClient()
            tasks.append(asyncio.create_task(run_step(step, client, console, junit, html, semaphore)))

    results = await asyncio.gather(*tasks)
    success = all(results)
    junit.write()
    html.summary()  # Generate HTML report
    prom_push()
    if console.summary():
        return 0
    return 1
